...
function armijorule(objective, gradx0, alpha, x0, x1)
    unitvec = -grad / sqrt(sum(grad^2))
    objective(x1) <= objective(x0) + 10e-4 * alpha * unitvec' * grad
end
...
function maparmijomax(objective, constraints, lb, ub, x0, alpha, tolerance)
    ...
    while (sum(abs(errors)) > tolerance)
        ...
        passes = false
        while ~passes
            x1 = linprog(-grad, A, '<', b, x0 - alpha, x0 + alpha) # XXX: assumes linprog minimizes
            passes = all(map(constraint -> x1, constraints))
...
@time sol2 = maparmijomax(testobjective, [mm -> mm[1] + mm[2] - 2], [0, 0], [2, 2], [1., 1.], .5, 1e-6)
println(sol2)
